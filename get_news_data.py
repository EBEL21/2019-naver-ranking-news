...
def article_threading(section_id):
    ...
    start_date = date(2019, r_date[0], r_date[1]) + timedelta(days=1)
...
def get_article_inner_info(article_url, driver):
    ...
    WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CLASS_NAME, 'u_likeit_layer')))
    ...
    try:
        ...
        WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.ID, 'cbox_module')))
        article_html = driver.page_source
        a_soup = BeautifulSoup(article_html, 'html.parser')
